...
function AjaxFunction(t) {
  ...
  ((a = (a = a + "?email=" + t) + "&sid=" + Math.random()),
    (e.onreadystatechange = function () {
      4 == e.readyState &&
        (document.getElementById("msg_email").innerHTML = e.responseText);
    }),
    e.open("GET", a, !0),
    e.send(null));
}
...
function formatTimeDisplay(seconds, display) {
  ...
  if (disp == 2) {
    var timer = numseconds < 10 ? "0" + numseconds : numseconds;
  }
  return timer;
}
function fetchRecordCount(
  ...
  if (typeof p2 !== "undefined" && typeof c2 !== "undefined") {
    count_url = count_url + "&p2=" + p2 + "&c2=" + c2;
  }
  if (typeof p3 !== "undefined" && typeof c3 !== "undefined") {
    count_url = count_url + "&p3=" + p3 + "&c3=" + c3;
  }
  $.ajax({
    url: count_url,
    type: "GET",
...
function check_user_name_avail(ajax_url, element_id, status_id, type) {
  ...
  if (type == "forgot_password") {
    var append = "&go=forgot";
  }
  if (type == "change_email") {
    var append = "&go=change";
  }
  jQuery.ajax({
...
function check_valid_email(ajax_url, element_id, status_id) {
  ...
  checkEmailTimer = setTimeout(function () {
    const elementSelector = "#" + element_id;
    const statusSelector = "#" + status_id;
    const emailValue = jQuery(elementSelector).val();

    // Abort previous in-flight request
    if (checkEmailRequest && checkEmailRequest.readyState !== 4) {
    ...
    checkEmailRequest = jQuery.ajax({
      ...
      success: function (data) {
        jQuery(statusSelector).html(data);
      },
...
$(document).ready(function () {
  ...
  $(".modal-window-link").fancybox({
    ...
    smallBtn: true,
    buttons: ["close"],
    iframe: { attr: { scrolling: "auto" }, preload: true },
  });
